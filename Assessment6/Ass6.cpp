...
// Function to define own absolute function as abs is not overloaded for unsigned int
size_t absolute(int a)
{
	if (a < 0) {
		a = a * -1;
	}
...
public:
	...
	matrix(matrix&& a_matrix) noexcept;
	// Destructor
	~matrix() { delete[] matrix_data; };
	//Copy and Move Assignment operators
	matrix& operator=(matrix& a_matrix);
	matrix& operator=(matrix&& a_matrix) noexcept;
...
matrix::matrix(matrix&& a_matrix) noexcept
{
	rows = a_matrix.rows;
...
matrix& matrix::operator=(matrix&& a_matrix) noexcept
{
...
matrix matrix::minor(size_t row_to_delete, size_t column_to_delete) const
{
	...
	if (row_to_delete > rows || row_to_delete < 1 || column_to_delete > columns || column_to_delete < 1 || rows != columns || rows < 2 || columns < 2) {
		...
		for (size_t index{1}; index < rows * columns + 1; index++) {
			...
			if (absolute(index - column_to_delete) % columns == 0) {
				continue;
			}
